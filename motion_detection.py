        ...
        if time_elapsed > 1./frame_rate:
            ...
            if detection > -1:
                ...
                if failedDetection > failedDetectionLimit and failedDetection < (failedDetectionLimit + 10) :
                    ...
                    awsPost = sproc.run('scp -o StrictHostKeyChecking=no -i (AWS PEM KEY) tempRecordings/'+recordingBatchID+str(recordingIndex)+'.avi (AWS USER)@'+dns+':/var/www/html/videos && rm tempRecordings/'+recordingBatchID+str(recordingIndex)+'.avi', shell=True, capture_output=True)
                    print(awsPost)
